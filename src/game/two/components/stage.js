...
class Stage extends React.Component {
	...
	getChildren = () => {
		...
		return <AppContext.Provider value={this.app}>{children}</AppContext.Provider>
	}

	updateSize = () => {
		if (this._container && this.app) {
			const rect = this._container.getBoundingClientRect()
			this.app.renderer.setSize(rect.width, rect.height, this.app.renderer.ratio)
		}
	}

	componentDidMount() {
		const { width, height, type, fitted, imageSmoothingEnabled, autostart } = this.props
		const params = {
			width,
			height,
			fitted,
	...
	render() {
		return <div id="tworender" style={this.props.style} ref={(c) => (this._container = c)}></div>
	}
}
